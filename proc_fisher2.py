...
def prof_fisher2(P, Pk, nstd=[], do_balance=False):
    ...
    F = np.zeros((np.shape(P)[1]))

    for fId in range(np.shape(P)[1]):

        #get data for given class and given feature (channel)
        cdata1 = P[np.any([Pk == classes[0]], axis=0), fId, :] #che forma ha questa cosa?
        cdata2 = P[np.any([Pk == classes[1]], axis=0), fId, :]
        ...
        F[fId] = np.absolute(m2-m1) / np.sqrt(s1**2 + s2**2)
